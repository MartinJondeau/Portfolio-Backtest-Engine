...
def get_realtime_asset(ticker: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail="Network Error: External API Unavailable"
        )
...
def backtest_sma(
    ...
    if period not in valid_periods:
        raise HTTPException(
            status_code=400, detail=f"Invalid period. Allowed: {valid_periods}"
        )
    if timeframe not in timeframe_mapping:
        raise HTTPException(status_code=400, detail="Invalid timeframe.")
    if short_window >= long_window:
        raise HTTPException(
            status_code=400, detail="Short window must be less than Long window"
        )

    # Fetch Data
    df = fetch_data_with_retry(
        ticker, period=period, interval=timeframe_mapping[timeframe]
    )
    ...
    processed_data["Date"] = pd.to_datetime(processed_data["Date"]).dt.strftime(
        "%Y-%m-%d"
    )
...
def backtest_mean_reversion(
    ...
    # Fetch Data
    df = fetch_data_with_retry(
        ticker, period=period, interval=timeframe_mapping.get(timeframe, "1d")
    )
...
def backtest_ml(
    ...
    if period not in valid_periods:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid period for ML. Needs history. Allowed: {valid_periods}",
        )
    if timeframe not in timeframe_mapping:
        raise HTTPException(status_code=400, detail="Invalid timeframe.")
    ...
    if len(df) < 100:
        raise HTTPException(
            status_code=400,
            detail=f"Not enough data points ({len(df)}) to train the model. Choose a longer period.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"ML Model Training Failed: {str(e)}"
        )
...
def get_portfolio_data(request: PortfolioRequest):
    ...
    if len(assets_data) < 3:
        raise HTTPException(
            status_code=404, detail="Could not fetch data for at least 3 valid tickers"
        )
...
def backtest_portfolio_with_strategies(request: dict):
    ...
    if len(assets_returns) < 3:
        raise HTTPException(
            status_code=404, detail="Could not process at least 3 assets"
        )
    ...
    for ticker in tickers:
        if ticker in assets_returns:
            ...
            if start_date:
                asset_returns_filtered = asset_returns_filtered[
                    asset_returns_filtered.index >= start_date_parsed
                ]
    ...
    result_df = pd.DataFrame(
        {"Date": returns_df.index, "Cumulative_Portfolio": portfolio_cumulative.values}
    )
...
def trigger_daily_report(background_tasks: BackgroundTasks):
    ...
    return {
        ...
        "message": "Check back in 10-20 seconds",
    }
