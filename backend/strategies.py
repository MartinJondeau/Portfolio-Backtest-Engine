...
def apply_ml_strategy(df):
    ...
    test_data["Cumulative_Market"] = (
        test_data["Cumulative_Market"] / test_data["Cumulative_Market"].iloc[0]
    )
    test_data["Cumulative_Strategy"] = (
        test_data["Cumulative_Strategy"] / test_data["Cumulative_Strategy"].iloc[0]
    )
...
def predict_future_prices(df, days=21):
    ...
    future_dates = pd.date_range(
        start=data.index[-1] + pd.Timedelta(days=1), periods=days, freq="B"
    )
