...
def simulate_portfolio(
    ...
    if rebalance_frequency == "never":
        ...
        portfolio_returns = rebalance_portfolio(
            returns, weights_array, rebalance_frequency
        )
...
def rebalance_portfolio(
    returns: pd.DataFrame, initial_weights: np.ndarray, frequency: str
) -> pd.Series:
    """
    ...
    for date in returns.index:
        ...
        if daily_return != -1:  # Avoid division by zero
            current_weights = (
                current_weights * (1 + returns.loc[date]) / (1 + daily_return)
            )
